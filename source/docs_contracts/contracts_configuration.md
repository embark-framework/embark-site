...
In Embark, you can describe how contracts related to each other, their parameters, and actions to run. Embark will automatically determine the best order to deploy contracts and execute the actions needed to reflect the state described.
If the contracts code or configuration changes, Embark will make only the necessary deployments to reflect the new state.
